        ...
        if (isset($data['username']) && isset($data['password']) && isset($data['versionCode']) && isset($data['token'])) {
            ...
            if (!empty($studentDetails)) {
                if ($studentDetails[0]['status'] == '1') {
                    ...
                    $fee_details = $this->db_model->select_data('*', 'student_payment_history', array(
                    ...
                    $this->db_model->update_data_limit('students use index (id)', array(
                        'login_status' => 1,
                        'app_version'  => $version,
                        'token'        => $token
                    ), array('id' => $studentDetails[0]['id']), 1);
                    ...
                    $checkLoginOTP = $this->general_settings('loginOTP');
                
                    if ($checkLoginOTP != 2) {
                        $userOtpId = $studentDetails[0]['id'];
                        ...
                        $otpExitCheck = $this->db_model->select_data('*', 'verification_login', array('user_id' => $userOtpId), 1);
            
                        if (!empty($otpExitCheck)) {
                            $this->db_model->update_data_limit('verification_login', $this->security->xss_clean($otp_data), array('id' => $otpExitCheck[0]['id']));
                        } else {
                            $otp_data['user_id'] = $userOtpId;
                            $this->db_model->insert_data('verification_login', $this->security->xss_clean($otp_data));
                        }

                        $studentData['loginOtp'] = $num;
                        $arr['studentData'] = $studentData;
                    }
	...
	function student_registration(){
			...
			if(!empty($data['name']) && !empty($data['email']) && !empty($data['mobile'])){
				...
				// 	die(); 
                        ...
                        $response = $this->SendMailSmtp($data['email'], $subj, $em_msg);
...
// 			echo $this->db->last_query();		die;	
        ...
        if(isset($data['admin_id'])){
            ...
            if(!empty($data['student_id'])){
             ...
             if(isset($data['v_id']) && !empty($data['v_id'])){
                 $con = array('admin_id'=>$admin_id,'id'=>$data['v_id'],'last_date >= '=> $currentDate,'status'=>1);
             }else{
    ...
    {   
        ...
        if(isset($data['batch_id'])){
            ...
            $datasdk = $this->db_model->select_data('*','live_class_setting',array('batch'=>$data['batch_id']))[0];
    ...
    function get_all_data(){
      ...
      if(isset($data['length']) && $data['length']>0){
         ...
         $slider_limit = $data['limit'];
         $batch_Data = array('status'=>1,'id'=>$data['batch_id']);
         $batch_cat = $this->db_model->select_data('id, batch_name as batchName, start_date as startDate, end_date as endDate, start_time as startTime, end_time as endTime, batch_type as batchType, batch_price as batchPrice, no_of_student as noOfStudent ,status,description, batch_image as batchImage, batch_offer_price as batchOfferPrice','batches use index (id)',$batch_Data,$limit,array('id','desc'));
                    ...
                    if(!empty($search)){
                      $like_search = array('batch_name',$search);
                       $cond_sub1='';
                        ...
                        $like_search = '';
                        //$cond_sub1 = array('status'=>1,'sub_cat_id'=>$value['SubcategoryId']);
                        $cond_sub1 = array('status' => 1,'sub_cat_id' => $value['SubcategoryId'],'id' =>$data['batch_id']);
                    }
                
                   
                $batchData = $this->db_model->select_data('id, batch_name as batchName, start_date as startDate, end_date as endDate, start_time as startTime, end_time as endTime, batch_type as batchType, batch_price as batchPrice, no_of_student as noOfStudent ,status,description, batch_image as batchImage, batch_offer_price as batchOfferPrice','batches use index (id)',$cond_sub1,$limit,array('id','desc'),$like_search);
                $bprice = !empty($batchData[0]['batchPrice'])?$batchData[0]['batchPrice']:0;
                $oprice = !empty($batchData[0]['batchOfferPrice'])?$batchData[0]['batchOfferPrice']:0;
                if(!empty($bprice) && !empty($oprice)){
                    $cal1 = $bprice-$oprice;
                    $cal2 = $cal1/$bprice;
                    $deff = $cal2*100;
                    $batchData[0]['Percentage'] = $deff;
                }else{
                     $batchData[0]['Percentage'] = 0;
                }
                
                  if($batchData[0]['batchPrice']==''){
                     $batchData[0]['batchPrice']='free';
                  } 
...
//     }
    ...
    function SendMailSmtp($to='', $subject = '', $body = ''){
        ...
        $smtp_user   	= 	 $this->general_settings('smtp_mail');
        $smtp_pass   	= 	 $this->general_settings('smtp_pwd');
  ...
  function  order_id_gen(){
    ...
    function verify_otp() {
        ...
        if (!empty($user_id) && !empty($otp)) {
            $otp_row = $this->db_model->select_data('*', 'verification_login', array('user_id' => $user_id), 1);
    
            if (!empty($otp_row)) {
                if ($otp_row[0]['otp'] == $otp) {
                     echo json_encode(['status' => 'true', 'msg' => 'OTP is verified']);
                } else {
                    echo json_encode(['status' => 'false', 'msg' => 'OTP not matched']);
                }
            } else {
                echo json_encode(['status' => 'false', 'msg' => 'OTP not found']);
            }
        } else {
            echo json_encode(['status' => 'false', 'msg' => 'Post request is empty']);
        }
    }
...
function resend_otp() {
    ...
    if (!empty($user_id)) {
        ...
        if (!empty($otp_row)) {
            $this->db_model->update_data_limit('verification_login', $this->security->xss_clean($otp_data), array('id' => $otp_row[0]['id']));
        } else {
            $otp_data['user_id'] = $user_id;
            $this->db_model->insert_data('verification_login', $this->security->xss_clean($otp_data));
        }

        echo json_encode(['status' => 'true', 'msg' => 'OTP generated', 'otp' => $num]);
    ...
    //     $otp = $this->input->post('otp');
    //     if (!empty($user_id) && !empty($otp)) {
    //         $num_rows = $this->db_model->countAll('verification_login', array('user_id' => $user_id));
    //         if ($num_rows == 1) {
    //             $user_id_info = $this->db_model->select_data('*', 'verification_login', array('user_id' => $user_id), 1);
    //             $currTime = date('Y-m-d H:i:s');
    //             $verification_login = $this->db_model->select_data('*', 'verification_login', array('user_id' => $user_id, 'otp' => $otp), 1);
    //             if (!empty($verification_login) && $verification_login[0]['otp'] == $otp) {
    //                 if ($verification_login[0]['create_at'] >= date('Y-m-d H:i:s', strtotime('-5 minutes', strtotime($currTime)))) {
    //                     $otpveryfy = "1";
    //                 } else {
    //                     $otpveryfy = "2";
    //                 }
    //                 if ($otpveryfy == 2 || $otpveryfy == 0) {
    //                     $j_arr = array('status' => 'false', 'msg' => $this->lang->line('ltr_otp_expire'));
    //                     $json_signup = json_encode($j_arr);
    //                     echo $json_signup;
    //                 } else {
    //                     $j_arr = array('status' => 'true', 'msg' => 'otp is verified');
    //                     $json_signup = json_encode($j_arr);
    //                     echo $json_signup;
    //                 }
    //             } else {
    //                 $j_arr = array('status' => 'false', 'msg' => 'otp not matched');
    //                 $json_signup = json_encode($j_arr);
    //                 echo $json_signup;
    //             }
    //         } else {
    //             $j_arr = array('status' => 'false', 'msg' => 'otp is not verified');
    //             $json_signup = json_encode($j_arr);
    //             echo $json_signup;
    //         }
    //     } else {
    //         $j_arr = array('status' => 'false', 'msg' => 'post request is empty');
    ...
    //         if ($num_rows == 1) {
    //             $num = rand(100000, 999999);
    //             $studentDetails = $this->db_model->select_data('*', 'students use index (id)', ['id'=>$user_id], 1);
    //             $msg_data['loginOtp'] = $num;
    //             $msg_data['number'] = $studentDetails[0]['contact_no'];
    //             $msg_data['name'] = $studentDetails[0]['name'];
    //             $this->load->library('otp');
    //             $twilio = $this->otp->SendOTP($msg_data);
    //             $otpRes = json_decode($twilio, true);
    //             if (!empty($otpRes) && $otpRes['status'] == 1) {
    //                 $otpExitCheck = $this->db_model->select_data('*', 'verification_login', array('user_id' => $user_id), 1);
    //                 if (!empty($otpExitCheck)) {
    //                     $otpUpdate['otp'] = $num;
    //                     $otpUpdate['create_at'] = date('Y-m-d H:i:s');
    //                     $ins = $this->db_model->update_data_limit('verification_login', $this->security->xss_clean($otpUpdate), array('id' => $otpExitCheck[0]['id']));
    //                 } else {
    //                     $otpinsert['otp'] = $num;
    //                     $otpinsert['user_id'] = $user_id;
    //                     $ins1 = $this->db_model->insert_data('verification_login', $otpinsert);
    //                 }
    //                 $j_arr = array('status' => 'true', 'msg' => 'otp sent');
    //                 echo json_encode($j_arr);
    //             } else {
    //                 $j_arr = array('status' => 'false', 'msg' => $this->lang->line('ltr_something_msg'));
    //                 echo json_encode($j_arr);
    //             }
    //         } else {
    //             $j_arr = array('status' => 'false', 'msg' => 'otp is not verified');
